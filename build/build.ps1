...
Param(	
    ...
    [string[]]$ScriptArgs,
	[string[]]$SkipExtraFilesOnServer="true",
	[string]$Site="core-dev",
	[string]$Domain="boldbi"
)
...
$isHomeEnvAvailable=Test-Path Env:\Home
if($isHomeEnvAvailable){
...
try{
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -target=`"$Target`" -publishProfilefilepath=`"$publishProfilefilepath`" -password=`"$password`" -configuration=`"$Configuration`" -PreReleaseNumber=`"$revisionNumber`" -studio_version=`"$StudioVersion`" -nugetserverurl=`"$nugetserverurl`" -verbosity=`"$Verbosity`" -nugetapikey=`"$nugetapikey`" -outputpath=`"$OutputPath`" -SkipExtraFilesOnServer=`"$SkipExtraFilesOnServer`" -referencepath=`"$ReferencePath`" -domain=`"$Domain`" -site=`"$Site`" -GitUserName=`"$gitusername`" -GitPassword=`"$gitpassword`" $ScriptArgs"
}
catch{
