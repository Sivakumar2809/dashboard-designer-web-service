...
bbicustom.dashboard.registerWidget({
    ...
    guid:"3e34d29e-baa7-426d-9646-57e3af402c21",

    widgetName:"CustomPivotChart",
		...
		this.pivotObj = new ej2chart.pivotview.PivotView({
			...
			chartSettings: {
				...
				chartSeries: { 
					...
					fill: ((this.model.properties.chartType == 'StackingColumn' || !this.model.properties.cchartColor) ? "" : this.model.properties.chartColor),
					width: 3
				},
				zoomSettings:{
					toolbarItems: []
				...
				seriesRender: $.proxy(this.seriesRender, this)
			},
	...
	seriesRender:function(args){
		args.series.legendShape = this.model.properties.legendShape;
	},
			...
			if(args.text.indexOf(this.model.boundColumns.value[i].columnName) > -1){
				...
				for(var j = 0; j < splittext.length; j++){
					if(splittext[j].indexOf(this.model.boundColumns.value[i].columnName) > -1){
						...
						var measureFormat = widgetInstance.dataGroupInfo.FieldContainers[0].FieldInfos[i].MeasureFormatting;
						var designerObj = $("#" + this.designId).data("BoldBIDashboardDesigner");
						var formattedText = BoldBIDashboard.DashboardUtil.formattedText((value.replaceAll(',','')),measureFormat.Culture, measureFormat.DecimalPoints, measureFormat.FormatType, measureFormat.DecimalSeparator, measureFormat.GroupSeparator, measureFormat.Prefix, measureFormat.Suffix, measureFormat.Unit, false, designerObj);
	...
	axisLableCustomization : function(args){
		if(this.model.boundColumns.value.length > 0) {
			var widgetInstance = $(this.element).closest(".e-customwidget-item").data("widgetInstance");
			var measureFormat = widgetInstance.dataGroupInfo.FieldContainers[0].FieldInfos[args.axis.columnIndex].MeasureFormatting;
			if(args.axis.orientation == "Vertical"){
				var designerObj = $("#" + this.designId).data("BoldBIDashboardDesigner");
				args.text = BoldBIDashboard.DashboardUtil.formattedText(args.value, measureFormat.Culture, measureFormat.DecimalPoints, measureFormat.FormatType, measureFormat.DecimalSeparator, measureFormat.GroupSeparator, measureFormat.Prefix, measureFormat.Suffix, measureFormat.Unit, false, designerObj);
