...
try{
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -target=`"$Target`" -publishProfilefilepath=`"$publishProfilefilepath`" -configuration=`"$Configuration`" -PreReleaseNumber=`"$revisionNumber`" -studio_version=`"$StudioVersion`" -nugetserverurl=`"$nugetserverurl`" -verbosity=`"$Verbosity`" -nugetapikey=`"$nugetapikey`" -outputpath=`"$OutputPath`" -SkipExtraFilesOnServer=`"$SkipExtraFilesOnServer`" -referencepath=`"$ReferencePath`" -site=`"$Site`" $ScriptArgs"
}
catch{
