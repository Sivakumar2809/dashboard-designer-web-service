node('BoldBI')
{
   ...
   if(githubSourceBranch == 'master')
    ...
    else if(githubTargetBranch.toLowerCase().contains('release/cloud-master'))
    {
       env.nugetserverurls = 'http://nexus.syncfusion.com/repository/boldbi-release/,https://api.nuget.org/v3/index.json';
    }
    stage 'Checkout' 
    try
    {     
